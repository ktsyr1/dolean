        ...
        <section>
            ...
            <div className="flex items-center my-4  ">
                <div className="relative flex-col justify-center flex">
                    ...
                    <Link to="/courses" className="inline-flex justify-center items-center py-3 px-8 text-base font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 cursor-pointer w-max mx-auto"> المزيد من الدورات </Link>
                </div>
            </div>
        </section>
    );
...
function Card({ data, className }) {
    ...
    return (
        <Link to={`/courses/${_id}`} className={`flex flex-none flex-col justify-between w-full bg-white m-4 max-w-[360px] min-w-[300px] border-gray-200 rounded-lg shadow ddark:bg-gray-800 ddark:border-gray-700 ${className}`}>
            {image.length > 1
                ? <img loading='lazy' className=" rounded-lg w-auto sh-[300px] " src={image} alt={title} />
            ...
            <div className="p-5">
                <h5 className="text-xl font-semibold tracking-tight text-gray-900 ddark:text-white">{title}</h5>
