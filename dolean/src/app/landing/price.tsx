...
const PricingCard = ({ title, description, price, list = [], to, textLink }: any) => (
    <div className="flex flex-col p-6 mx-auto max-w-lg- text-center text-gray-900 bg-white rounded-lg border border-gray-100 shadow ddark:border-gray-600 xl:p-8 ddark:bg-gray-800 ddark:text-white max-w-[400px]">
        ...
        <Link href={to} className={`${typeof price == "string" ? "bg-white text-primary-600" : "text-white bg-primary-600"}   focus:ring-4 focus:ring-primary-200 font-medium rounded-lg border  border-primary-600  text-sm px-5 py-2.5 text-center ddark:text-white ddark:focus:ring-primary-900`}>{textLink}</Link>
    </div>
);
